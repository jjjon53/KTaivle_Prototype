...
class PKSmartInference:
    ...
    def load_models(self):
        ...
        print(
            f"Models loaded. LD50 Model: {'Loaded' if self.ld50_model else 'Not Found'}"
        )
    ...
    def predict(self, smiles: str):
        ...
        if self.tox_models:
            ...
            for task, model in self.tox_models.items():
                ...
                result["tox21"][task] = {"probability": prob, "risk": risk}
        ...
        if self.ld50_model:
            ...
            ld50_val = (
                10 ** float(y_pred_log) - 1
            )  # Use formula from pksmart_predict.py

            # Classification Logic (GHS) matching pksmart_predict.py
            tox_class = "VI"
            tox_label = "Non-toxic"
            if ld50_val <= 5:
                tox_class, tox_label = "I", "Fatal"
            elif ld50_val <= 50:
                tox_class, tox_label = "II", "Fatal"
            elif ld50_val <= 300:
                tox_class, tox_label = "III", "Toxic"
            elif ld50_val <= 2000:
                tox_class, tox_label = "IV", "Harmful"
            elif ld50_val <= 5000:
                tox_class, tox_label = "V", "May be harmful"
...
def run_cohort_simulation(
    ...
    """
    print(
        f"Running Cohort Sim: Pop={pop}, N={n_subjects}, CL={cl_ml_min_kg}, Vd={vd_l_kg}"
    )
...
def run_dose_response_analysis(
    ...
    compare_doses: list = None,
):
    """
    Runs a lightweight dose-response analysis for the Heatmap and PK Comparison.
    Simulates dose levels using a small representative cohort.

    Args:
        compare_doses: Optional list of specific doses to simulate (e.g. [50, 100, 150, 200])
                       If None, uses default multipliers of base_dose_mg.
    """
    results = []

    if compare_doses:
        doses = sorted(list(set(compare_doses)))  # Unique and sorted
    else:
    ...
    for dose in doses:
        drug = DrugParams(dose_mg=float(dose), MW_kDa=mw_kda)
        ...
        if hasattr(sims, "__iter__") and len(sims) > 0:
            ...
            # Cmax: nM -> ng/mL (using MW)
            # AUC: nM*day -> ng*hr/mL (nM * MW = ng/mL, day->hr = *24)

            cmax_vals = [s.get("C_max", 0) for s in sims]
            auc_vals = [s.get("AUC", 0) for s in sims]

            # Convert units
            # nM = nmol/L. MW = g/mol.
            # 1 nM * MW(g/mol) = 1 nmol/L * g/mol = 1 ng/mL
            cmax_ng_ml = np.mean(cmax_vals) * mw_g_mol
            auc_ng_hr_ml = np.mean(auc_vals) * mw_g_mol * 24.0

            mean_to = np.mean([s.get("mean_to", 0) for s in sims])

            # Tmax estimation (from first subject for simplicity, or average index)
            # Time array is in 'time' key
            tmax_vals = []
            for s in sims:
                if "time" in s and "C_plasma" in s:
                    t_idx = np.argmax(s["C_plasma"])
                    tmax_vals.append(s["time"][t_idx] * 24.0)  # Convert days to hours
            mean_tmax = np.mean(tmax_vals) if tmax_vals else 0.0

            # Theoretical t1/2 (Linear assumption, dose independent)
            # t1/2 = 0.693 * Vd / CL
            # Vd (L), CL (L/day) -> CL (L/hr) = CL/24
            # t1/2 (hr) = 0.693 * Vd / (CL/24) = 16.632 * Vd / CL
            t_half = 16.632 * vd_L / cl_L_day if cl_L_day > 0 else 0.0

            results.append(
                {
                    "dose": round(dose, 1),
                    "mean_to": round(mean_to, 1),
                    "cmax": round(cmax_ng_ml, 1),
                    "auc": round(auc_ng_hr_ml, 1),
                    "tmax": round(mean_tmax, 1),
                    "t_half": round(t_half, 1),
                }
            )
